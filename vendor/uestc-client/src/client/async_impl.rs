...
impl UestcClient {
    ...
    pub async fn wechat_login(&self) -> Result<()> {
        ...
        let resp = self.client.get(&cas_login_url).send().await?;
        ...
        let resp = self.client.get(&xml_url).send().await?;
    ...
    /// 切换成功后 `ctx` 的当前方式随之更新。
    pub async fn change_reauth_type(
        &self,
        ctx: &mut ReauthContext,
        method: &ReauthMethod,
    ) -> Result<()> {
    ...
    pub async fn send_reauth_code(&self, ctx: &ReauthContext, method: &ReauthMethod) -> Result<()> {
    ...
    async fn submit_reauth_form(
        ...
        self.finish_to_service(ctx).await
    }
    ...
    async fn submit_wechat_reauth(&self, ctx: &ReauthContext, skip_tmp_reauth: bool) -> Result<()> {
        ...
        let uuid = match wechat::parse_qr_uuid_from_xml(&xml_text) {
            ...
            Err(xml_err) => {
                ...
                let resp = self.client.get(&html_url).send().await?;
                let html = resp.text().await?;
    ...
    async fn finish_to_service(&self, ctx: &ReauthContext) -> Result<()> {
